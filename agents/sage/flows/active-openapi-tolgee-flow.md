# Active OpenAPI and Tolgee Foundation Flow

> Design record for turning the documented OpenAPI and Tolgee recipes into executable, domain-neutral template foundations.
...
- Use locale-prefixed URLs (`/th`, `/en`) through `next-intl`; Thai is the fallback locale.
...
[Clone + pnpm install]
          ...
          |
          +--> GET / --> proxy negotiates locale --> /th or /en
                                                        |
...
2. `src/proxy.ts` redirects unprefixed routes and excludes APIs, Next internals, and files.
...
6. The language switcher replaces the current pathname with another locale via locale-aware navigation.
...
| API base URL is empty | Fail client construction with a clear local error |
...
- [x] `ACTIVE-04` Routing — `/` redirects to a supported locale and invalid locales reach not-found.
- [x] `ACTIVE-05` Documentation — README and recipes describe the active paths and pull-to-repository workflow.
- [x] `ACTIVE-06` Validation — generation check, format, lint, typecheck, tests, build, audit, and browser smoke checks pass.
...
- Adding TanStack Query, mutation hooks, Redux, forms, database, or authentication.
