...
export default async function HomePage() {
  ...
  return (
    <main className='relative min-h-svh overflow-hidden bg-background text-foreground'>
      ...
      <div className='relative mx-auto flex min-h-svh w-full max-w-6xl flex-col px-6 py-8 sm:px-10 sm:py-10 lg:px-12'>
        ...
        <section className='grid flex-1 items-center gap-14 py-16 lg:grid-cols-[1.25fr_0.75fr] lg:gap-24 lg:py-20'>
          <div>
            <p className='mb-6 font-mono text-xs font-medium tracking-[0.2em] text-muted-foreground uppercase'>
        ...
        <footer className='flex flex-col gap-2 border-t border-border pt-5 font-mono text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between'>
          <p>{t('common:home.footer.ownership', 'เป็นเจ้าของ component source และเพิ่ม dependency เท่าที่จำเป็น')}</p>
          <p>pnpm · TypeScript · OpenAPI · Tolgee</p>
        </footer>
      </div>
    </main>
  )
